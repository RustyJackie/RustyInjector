...
def preflight(method: str, require_root: bool, abort_on_monitors: bool) -> None:
    ...
    if missing:
        die(
            ...
            f"      Install:  {DIM}sudo apt install {' '.join(missing)}{RST}"
        )
...
def find_process_pid(process_name: str) -> str:
    ...
    if len(pids) > 1:
        warn(
            f"Multiple instances: {', '.join(pids)}\n"
...
def verify_injection(pid: str, library: Path, used_memfd: bool) -> None:
    """
